...
public interface SeatRepository extends JpaRepository<SeatEntity,String> {
    ...
    @Modifying
    @Query("UPDATE SeatEntity s SET s.status = 'Available', s.userId = null, s.expiry = null " +
            "WHERE s.status = 'Held' AND s.expiry < :currentTime")
    int releaseExpiredSeats(@Param("currentTime") long currentTime);
}
